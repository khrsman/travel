...
class M_booking extends CI_Model
{
    ...
    public function insert($data,$tanggal_unit){
      ...
      foreach ($tanggal_unit as $key => $value) {
        foreach ($value as $keyy => $valuee) {
          ...
          $detail_booking[$key]['id_vendor'] = $data['id_vendor'];
        }
      }
    ...
    public function update_harga($data){
      ...
      // die;
        $this->db->trans_start();
        $this->db->update_batch('detail_booking',$data, 'id');
    ...
    public function get_detail_harga($id){
         $this->db->select("
          ...
          harga_pokok harga_pokok, (select sum(harga) from detail_booking where id_booking = bk.id_booking) harga_jual")->where('id_booking',$id);
