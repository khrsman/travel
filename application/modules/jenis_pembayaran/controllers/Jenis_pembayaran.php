...
class Jenis_pembayaran extends CI_Controller {
    ...
    // view
    function index(){
    $data['page'] = 'v_jenis_pembayaran';
    $this->load->view('v_main',$data);
    }

    function data(){
    $this->load->view('v_jenis_pembayaran_data');
    }

    function editor(){
    $this->load->view('v_jenis_pembayaran_editor');
    }
    ...
    public function page()
    {
      ...
      $data = $this->M_jenis_pembayaran->get_data($search,$from_page,$per_page); // ambil data
    ...
    function get_for_edit(){
      ...
      $data = $this->M_jenis_pembayaran->get($id);
    ...
    function add(){
        ...
        if (isset($data['id_jenis_pembayaran'])) {
          $data['id_jenis_pembayaran'] = NULL;
        };
        $insert = $this->M_jenis_pembayaran->insert($data);
        if (!$insert) {
    ...
    function update(){
      $data = array();
      parse_str($_POST['data'], $data);
      $id = $data['id_jenis_pembayaran'];
      $update = $this->M_jenis_pembayaran->update_by_id($data,$id);
    }

    // fungsi hapus
    function delete(){
      $id = $this->input->post('id');
      $delete = $this->M_jenis_pembayaran->delete_by_id($id);
    }
